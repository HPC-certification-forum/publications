...
\headerbox{Status}{name=next,column=2,below=pecoh}{
  ...
  \begin{itemize}
  \setlength\itemsep{-0.3em}
  \item A first skill-tree is created (we work on ADM/BDA trees)
  \item Skills are stored in an XML document and can be processed via XSLT into various representations; they are also available as Markdown.
  ...
  \item We established a set of rules of the forum and held a vote for the first steering board.
  \item A prototype for a first online examination is developed and evaluated.
  \end{itemize}
